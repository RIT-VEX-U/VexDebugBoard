...
namespace
{
  const char *WS_TAG = "ws-foxglove-server";
...
namespace foxglove
{
  ...
  static const char *RECOVERABLE = "RECOVERABLE";
  ...
  inline Server::Server(std::string name, LogCallback logger,
      ...
      : _name(std::move(name)), _logger(logger), _options(options)
  {
    // Redirect logging
  ...
  inline void Server::setHandlers(ServerHandlers<ConnHandle> &&handlers)
  {
    _handlers = handlers;
  }
  ...
  inline void Server::start(const std::string &host, uint16_t port)
  {
    if (_serverThread)
    {
  ...
  inline void Server::sendBinary(ConnHandle hdl, const uint8_t *payload,
                                 size_t payloadSize)
  {
    _server.send(hdl, payload, payloadSize, OpCode::BINARY);
  }

  inline void Server::sendStatusAndLogMsg(ConnHandle clientHandle,
  ...
  inline void Server::handleTextMessage(ConnHandle hdl, MessagePtr msg)
  {
  ...
  inline void Server::handleBinaryMessage(ConnHandle hdl, MessagePtr msg)
  {
  ...
  inline std::vector<ChannelId> Server::addChannels(
  ...
  inline void Server::removeChannels(const std::vector<ChannelId> &channelIds)
  {
    if (channelIds.empty())
    {
      return;
  ...
  inline void Server::publishParameterValues(
  ...
  inline void Server::updateParameterValues(
  ...
  inline std::vector<ServiceId> Server::addServices(
  ...
  inline void Server::removeServices(const std::vector<ServiceId> &serviceIds)
  {
    std::unique_lock<std::shared_mutex> lock(_servicesMutex);
  ...
  inline void Server::broadcastMessage(ChannelId chanId, uint64_t timestamp,
  ...
  inline void Server::sendMessage(ConnHandle clientHandle, ChannelId chanId,
  ...
  inline void Server::broadcastTime(uint64_t timestamp)
  {
  ...
  inline void Server::sendServiceResponse(ConnHandle clientHandle,
  ...
  inline uint16_t Server::getPort()
  {
    websocketer::error_code ec;
  ...
  inline void Server::updateConnectionGraph(
  ...
  inline std::string Server::remoteEndpointString(ConnHandle clientHandle)
  {
    websocketer::error_code ec;
    const auto con = _server.get_con_from_hdl(clientHandle, ec);
  ...
  inline bool Server::isParameterSubscribed(const std::string &paramName) const
  {
  ...
  inline void Server::unsubscribeParamsWithoutSubscriptions(
  ...
  inline bool Server::hasCapability(const std::string &capability) const
  {
    return std::find(_options.capabilities.begin(), _options.capabilities.end(), capability) !=
           _options.capabilities.end();
  }

  inline bool Server::hasHandler(uint32_t op) const
  {
    switch (op)
    {
    case SUBSCRIBE:
  ...
  void Server::handleSubscribe(const nlohmann::json &payload, ConnHandle hdl)
  {
  ...
  void Server::handleUnsubscribe(const nlohmann::json &payload, ConnHandle hdl)
  {
    std::unordered_map<ChannelId, SubscriptionId> clientSubscriptionsByChannel;
    {
      std::shared_lock<std::shared_mutex> clientsLock(_clientsMutex);
  ...
  void Server::handleAdvertise(const nlohmann::json &payload, ConnHandle hdl)
  {
  ...
  void Server::handleUnadvertise(const nlohmann::json &payload, ConnHandle hdl)
  {
    std::unique_lock<std::shared_mutex> clientChannelsLock(_clientChannelsMutex);
    auto clientPublicationsIt = _clientChannels.find(hdl);
    if (clientPublicationsIt == _clientChannels.end())
  ...
  void Server::handleGetParameters(const nlohmann::json &payload,
                                   ConnHandle hdl)
  {
  ...
  void Server::handleSetParameters(const nlohmann::json &payload,
                                   ConnHandle hdl)
  {
  ...
  void Server::handleSubscribeParameterUpdates(const nlohmann::json &payload,
                                               ConnHandle hdl)
  {
  ...
  void Server::handleUnsubscribeParameterUpdates(const nlohmann::json &payload,
                                                 ConnHandle hdl)
  {
    const auto paramNames = payload.at("parameterNames").get<std::unordered_set<std::string>>();
    {
  ...
  void Server::handleSubscribeConnectionGraph(ConnHandle hdl)
  {
  ...
  void Server::handleUnsubscribeConnectionGraph(ConnHandle hdl)
  {
  ...
  void Server::handleFetchAsset(const nlohmann::json &payload, ConnHandle hdl)
  {
  ...
  inline void Server::sendFetchAssetResponse(
